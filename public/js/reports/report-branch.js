...
{
    startMonthPicker = $("#startMonthPicker").kendoDatePicker({
        culture: "es-ES",
        value: moment().locale("en").startOf('month').format('MMMM YYYY'),
        start: "year",
        depth: "year",
        format: "MMMM yyyy",
        dateInput: true,
...
function initEndMonthPicker()
{
    endMonthPicker = $("#endMonthPicker").kendoDatePicker({
        culture: "es-ES",
        value: moment().locale("en").startOf('month').format('MMMM YYYY'),
        start: "year",
        depth: "year",
    ...
    endDate = endMonthPicker.value();
}

function startPickerChange()
{
    let momentStartDate = moment(startMonthPicker.value());
    let momentEndDate = moment(endMonthPicker.value());

    //alert(momentEndDate.diff(momentStartDate, 'months'));

    if(momentEndDate.diff(momentStartDate, 'months') > 3)
    {
        endMonthPicker.value(momentStartDate.add(2, 'month').locale("en").format('MMMM YYYY'));
    }

    if(momentEndDate.diff(momentStartDate, 'months') < 0)
    {
        endMonthPicker.value(momentStartDate.add(1, 'month').locale("en").format('MMMM YYYY'));
    }
...
function endPickerChange()
{
    ...
    if(momentEndDate.diff(momentStartDate, 'months') > 3)
    {
        startMonthPicker.value(momentEndDate.subtract(2, 'month').locale("en").format('MMMM YYYY'));
    }

    if(momentEndDate.diff(momentStartDate, 'months') < 0)
    {
        startMonthPicker.value(momentEndDate.subtract(1, 'month').locale("en").format('MMMM YYYY'));
    }
  ...
  gridBranch = $("#gridBranch").kendoGrid({
        dataSource: {
            ...
            requestEnd: function(e) {
                ...
                if(e.type == "read" && e.hasOwnProperty("response"))
                {
                    let total_evaluations = 0; 
                    let evaluations_expected = 0;

                    $.each(e.response, function(key, value) {
                        total_evaluations += value.applied_evaluation[0];
                        evaluations_expected += value.applied_evaluation[1];
                    });
        ...
        excelExport: function(e) {
            ...
            for (var i = 1; i < sheet.rows.length; i++)
            {
                ...
                row.cells[4].value = row.cells[4].value[0] + " de " + row.cells[4].value[1];
                row.cells[5].value = row.cells[5].value[0] + " de " + row.cells[5].value[1];
            }
        },
        toolbar: ["excel"],
        excel: {
            fileName: "Reporte de sedes.xlsx",
            filterable: false,
            allPages: true
        },
        height: "500px",
        ...
        [
            ...
            },
            {
                ...
                responsive: true,
                filterable: false,
                template: '#=formatBarFinalNote(data)#',
            },
            {
...
function initGridOptions()
{
    gridOptions = $("#gridOptions").kendoGrid({
        ...
        toolbar: ["excel"],
        excel: {
            fileName: "Reporte de sedes por sección.xlsx",
            filterable: false,
            allPages: true
        },
        height: "500px",
...
function formatBarFinalNote(data)
{
    return "<div class='progress' style='height:18px; margin-top:20px;' title=" + data.average_final + ">"+
                "<div class='progress-bar bg-success' role='progressbar' style='background-color:" + getColor(data.average_final) + "!important; width: " + data.average_final + "%' aria-valuenow='30' aria-valuemin='0' aria-valuemax='100'>" + data.average_final + "%</div>"+
           "</div>";
}
...
{   
    ...
    if(data.applied_evaluation[0] >= data.applied_evaluation[1])
    {
        color = "success";
        icon  = "<i class='fad fa-check-circle'></i>";
    }

    return "<span title='Códigos: " + data.evaluation_codes + "' class='badge badge-" + color + "' style='font-weight:500;font-size: 14px;'>" + icon + " " + data.applied_evaluation[0] + ' de ' + data.applied_evaluation[1] + "</span>";
}
