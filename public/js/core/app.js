  ...
  bookmarkStar.on("click", function (e) {
    ...
    for (var i = 0; i < arrList.length; i++) {
      ...
      $arrList +=
        ...
        "</span>" +
        "</div>" +
        '<span data-url='+ $(arrList[i].firstChild).data('url') +' class="float-right bookmark-icon feather icon-star warning"></span>' +
        "</a>" +
        "</li>";
    }
    ...
    if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
      ...
      if (value != "") {
        ...
        /*$.getJSON("data/" + $filename + ".json", function (
          ...
          let data = {};
          data['listItems'] = window.dataSearch;
          ...
          for (var i = 0; i < data.listItems.length; i++) {
            ...
            if (bookmark === true) {
              ...
              for (var j = 0; j < arrList.length; j++) {
                console.log(data.listItems[i].name, '|' + arrList[j].firstChild.dataset.originalTitle);
                if (
                  data.listItems[i].name.replace(/\s/g, '') ===
                  arrList[j].firstChild.dataset.originalTitle.replace(/\s/g, '')
                ) {
                  console.log("existe");
                  activeClass = " warning";
                  break;
              ...
              $bookmarkIcon =
                '<span data-url="'+ data.listItems[i].url +'" class="float-right bookmark-icon feather icon-star' +
                activeClass +
                '"></span>';
            }
            // Search list item start with entered letters and create list
            if (
              data.listItems[i].name.toLowerCase().replace(/\s/g, '').indexOf(value) == 0 &&
              a < 5
            ) {
              if (a === 0) {
                $activeItemClass = "current_item";
          ...
          for (var i = 0; i < data.listItems.length; i++) {
            if (bookmark === true) {
              ...
              // Loop to check if current seach value match with the bookmarks already there in navbar
              for (var j = 0; j < arrList.length; j++) {
                if (data.listItems[i].name.replace(/\s/g, '') === arrList[j].firstChild.dataset.originalTitle.replace(/\s/g, ''))
                {
                  activeClass = " warning";
                } else {
                  activeClass = "";
            ...
            if (
              !(data.listItems[i].name.toLowerCase().replace(/\s/g, '').indexOf(value) == 0) &&
              data.listItems[i].name.toLowerCase().replace(/\s/g, '').indexOf(value) > -1 &&
              a < 5
            ) {
              if (a === 0) {
                $activeItemClass = "current_item";
